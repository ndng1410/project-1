                ...
                <div class="modal-content">
                    <div class="modal-body">
                      <input
                        ...
                        <button type="button" class="btn btn-primary" data-bs-dismiss="modal" @click="onEdit(task.id, input)">Save changes</button>
                    </div>
                </div>
            </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    async onEdit(index, content) {
      try {
        await axios.put(`http://127.0.0.1:8000/api/tasks/${index}`, {task_name: content})
